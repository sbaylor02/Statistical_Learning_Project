...
```{r}
dat_1$Absent_time <- ifelse(dat_1$Absent_time == 0, 'good', 'bad')

#Examining variable 'Absent_time'
table(dat_1$Absent_time)
```

#eliminating column 1 since it does not provide any valuable information for prediction, 
#these are just variables that describe the data:
```{r}
dat_1 <- dat_1[,-c(1)]
dat_1[,c(5:10, 13)] <- scale(dat_1[,c(5:10, 13)])
```
