...
#bar chart
dat %>% ggplot(aes(x=ID))+geom_bar()+theme_minimal()
```
...
#bar chart
dat %>% ggplot(aes(x=Reason))+geom_bar()+theme_minimal()
```
...
#bar chart
dat %>% ggplot(aes(x=Month))+geom_bar()+theme_minimal()
```
...
ggplot(err_graph, aes(x=Var2, y=value))+geom_boxplot()
```

```{r}
sens_graph <- melt(sensitivity_matrix)

ggplot(sens_graph, aes(x=Var2, y=value))+geom_boxplot()
```

```{r}
fmeas_graph <- melt(fmeasure_matrix)

ggplot(fmeas_graph, aes(x=Var2, y=value))+geom_boxplot()
```

```{r}
gmean_graph <- melt(gmean_matrix)

ggplot(gmean_graph, aes(x=Var2, y=value))+geom_boxplot()
```
...
```{r}
graph <- ggplot(as.data.frame(sensitivity_matrix_opt),aes(x=KNN) )+ geom_boxplot(stat=identity)
graph
```
