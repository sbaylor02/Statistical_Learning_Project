...
#generate confusion matrix
 ...
 gmean_matrix [[r, 1]] <- sqrt(cm_KNN$byClass[1]* cm_KNN$byClass[2])
  ...
  ###glm Pavel code:
  model_glm_1 = train(Absent_time ~ .,
                      data = data_train,
                      method = "glm", 
                      family = 'binomial') 
  
  yhat_glm = predict(model_glm_1, newdata = data_test[,-21])
  
  cm_glm = confusionMatrix(data = yhat_glm, reference = data_test[,21])
  
  #generate confusion matrix
  #cm_glm <-  confusionMatrix(data = glmtable, reference = data_test[,-21], positive = "1")
...
colnames(glmcm) <- c("True Negative","False Positive", "False Negative","True Positive" )
```
...
data_test$Absent_time <- as.factor(data_test$Absent_time)
  ...
  #generate confusion matrix
 cm_tree <-  confusionMatrix(data = table(yhat_tree, data_test$Absent_time), reference = data_test[,-21], positive = "1")
